...
import wandb
from agents.cac import CAC, CACHP, data_func
from agents.utils.buffer import CACBuffer
...
if __name__ == "__main__":
    ...
    wandb.init(
        project="CAC", name=hp.EXP_NAME, entity="goncamateus", config=hp.to_dict()
    )
    ...
    for _ in range(hp.N_ROLLOUT_PROCESSES):
        data_proc = mp.Process(
            target=data_func,
            args=(cac, device, exp_queue, finish_event, sigma_m, gif_req_m, hp),
        )
        data_proc.start()
            ...
            while new_samples < hp.EXP_GRAD_RATIO:
                ...
                if isinstance(safe_exp, dict):
                    ...
                    buffer.add(
                        ...
                        councils_action=safe_exp.council,
